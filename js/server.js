...
io.on('connection', function(socket) {
    socket.on('joinGameEvent', function(data) {
        ...
        onePlayer = new Player(parseInt(data.player_id), "", "", 100, "", "", "inactive", 0, 0);
    ...
    socket.on('playerActionFold', function(data) {
        ...
        if(nrOfactivePlayers < 2){
            ...
            if(!isGameOn){ 
                startGame(socket, io);
            } else {//could it be 'fold' is hit but new game is started by new joined players
                console.log('new players are in.')
            }
        } else { // prompt next player
    ...
    socket.on('playerActionCall', function(data) {
        ...
        if(highestBet > player.subtotal_bet){
            ...
            console.log('this shouldnt happen that users: '+player.player_id+' bet is larger than highestBet.');
        }
    });
    ...
    socket.on('playerActionCheck', function(data) {
        ...
        player = getIdPlayer(parseInt(data.player_id));
        if(highestBet == player.subtotal_bet) {
            if(player.subtotal_bet != 0) { // big blind to start the flop
                dealCommunityCards();
                //reset highestBet and subtotal_bet to 0?
                for (var i = 0; i < playerList.length; i++) {
                    if (playerList[i].status == 'active') {
                ...
                button = getRolePlayers('button')[0];
                sendToPlayer(socketToPlayerMap, getNextPlayer(button));
            } else { //check done from small to button
                // new flag checked?
...
function updatePlayer(id, jTag, jVal){
    for (var i = 0; i < playerList.length; i++) {
        console.log('can this property name be working??? '+JSON.stringify(playerList[i]));
        if(playerList[i].player_id == id){
...
function getButtonPlayer(){
    ...
    if(buttonPlayer.length == 0){//new game, fresh player list no button assigned, or button just left the game?       
...
function Player (playerId, name, role, bankroll, carda, cardb, status, total_bet,
    subtotal_bet) {
